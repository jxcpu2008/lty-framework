...
<script type="text/javascript">
	$(function() {
		$('#admin_zygl_treegrid').treegrid({
			url : '${pageContext.request.contextPath}/resource/treegrid.shtml',
			idField : 'id',
			method: 'GET',
			treeField : 'text',
			parentField : 'pid',
			...
			buttons : [ {
				...
				handler : function() {
					...
					$('#admin_zyglAdd_addForm').form('submit', {
						url : '${pageContext.request.contextPath}/resource/add.shtml',
			...
			buttons : [ {
				...
				handler : function() {
					...
					$('#admin_zyglEdit_editForm').form('submit', {
						url : '${pageContext.request.contextPath}/resource/edit.shtml',
						success : function(result) {
							try {
								var r = $.parseJSON(result);
								if (r.success) {
	...
	function admin_zygl_deleteFun(id) {
		...
		if (node) {
			$.messager.confirm('询问', '您确定要删除【' + node.text + '】？', function(b) {
				if (b) {
					$.ajax({
						url : '${pageContext.request.contextPath}/resource/remove.shtml',
						data : {
							id : node.id
						},
						cache : false,
